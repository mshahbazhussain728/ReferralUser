...
export const SideBar = ({ isDrawer, handleDrawer }) => {
  ...
  // ✅ openTitle state lives HERE — initialized from localStorage so it survives refresh
  const [openTitle, setOpenTitle] = useState(() => {
    // First priority: check if current URL matches a child route — open that parent
    const matchedByUrl = sideBar.find((item) =>
      item.inner?.some((child) => location.pathname === child.pathname)
    );
    if (matchedByUrl) return matchedByUrl.title;

    // Second priority: restore last manually opened dropdown from localStorage
    return localStorage.getItem("sidebarOpenTitle") || null;
  });

  // ✅ Sync openTitle if the URL changes (e.g. browser back/forward)
  useEffect(() => {
    const matchedByUrl = sideBar.find((item) =>
      item.inner?.some((child) => location.pathname === child.pathname)
    );
    if (matchedByUrl && openTitle !== matchedByUrl.title) {
      setOpenTitle(matchedByUrl.title);
      localStorage.setItem("sidebarOpenTitle", matchedByUrl.title);
    }
  }, [location.pathname]);

  // ✅ Toggle a dropdown and persist the result
  const handleToggle = (item) => {
    setOpenTitle((prev) => {
      const next = prev === item.title ? null : item.title;
      if (next) {
        localStorage.setItem("sidebarOpenTitle", next);
      } else {
        localStorage.removeItem("sidebarOpenTitle");
      }
      return next;
    });
  };
